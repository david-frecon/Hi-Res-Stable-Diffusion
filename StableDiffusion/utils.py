...
@torch.no_grad()
def test_DDPM_chain(model, beta, max_t, shape=(1, 1, 28, 28), n_samples=4, save_video=False):
    big_chain = []
    ...
    for i in range(n_samples):
        for j, t in enumerate([-16, -8, -4, -2, -1]):
            ax[i, j].imshow(denormalize_img(big_chain[i][t].permute(0, 2, 3, 1).detach().cpu().squeeze()))
            ax[i, j].axis("off")
